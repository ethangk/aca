...
execute.prototype.emptyUnits = function(){
	for(var type in this.units){
		if(this.units.hasOwnProperty(type)){
			for(var i = 0; i<this.units[type].length; i++){
				var value = this.units[type][i].getResult();
				if(!!value){
					var b = this.units[type][i].dest;
					if(b === this.latestStore){
						//latest store is done, lets update
						for(var k = this.latestStore+1; k<reorderBuffer.values.length; k++){
...
execute.prototype.run = function(){
	...
	while(unit){
		...
		if(!decodedInstruction.or1 && !decodedInstruction.or2){
		...
		console.log("PLACING", decodedInstruction, unit);
