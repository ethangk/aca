...
instructionFetch.prototype.run = function(){
	...
	if(reorderBuffer.valuesLength - reorderBuffer.getPointer() >= config.reorderBufferSize){
		// if(config.debug){
			// console.log("reorder buffer not empty, stalling", reorderBuffer.values);
			// throw "stalling";
			stall = true;
			return;
		// }
	...
	if(this.bufferDE.values.length === config.issueWidth){
		...
		if(config.debug){
			console.log("decode buffer not empty, stalling", this.bufferDE.values);
		}
	...
	if(PCBus.fetch() > labels['end']+config.instructionLength && reorderBuffer.getItem(reorderBuffer.getPointer()) === undefined){
		finished = true;
	}
	...
	for(var i = this.bufferDE.values.length; i<config.issueWidth; i++){
		...
		if(PCBus.fetch() > labels['end'] && reorderBuffer.getPointer()-reorderBuffer.valuesLength === 0){
			// finished = true;
			// console.log("Breaking out of fetch");
			break;
		}
