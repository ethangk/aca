...
var pipeline = {IF: null, DC: null, EX: null, MA: null, WB: null};
var clock 	= 0;
...
function tick(IF,DC,EX,MA,WB){
	...
	var returnState =	WB.run(MA.currentInstruction);
						...
						EX.run(DC.currentInstruction);
						DC.run(IF.currentInstruction);
						IF.run();
	pipeline.IF = IF.currentInstruction;
	pipeline.DC = DC.currentInstruction;
	pipeline.EX = EX.currentInstruction;
	pipeline.MA = MA.currentInstruction;
	pipeline.WB = WB.currentInstruction;
	clock++;
	return	!(	IF.currentInstruction.opCode == 'hlt' && DC.currentInstruction.opCode == 'hlt' &&
